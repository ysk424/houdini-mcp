...
- **Headless auto-launch**: if no Houdini is listening, `find_hython()` locates Steam Houdini Indie's `hython.exe` and `_launch_headless_houdini()` spawns `scripts/headless_server.py` in a managed subprocess. Cleaned up on shutdown via lifespan + atexit. Disable with `HOUDINIMCP_NO_HEADLESS=1`.
